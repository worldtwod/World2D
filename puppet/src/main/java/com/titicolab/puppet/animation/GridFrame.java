...
public class GridFrame extends GroupFrame {
    ...
    GridFrame setLeftTop(float left, float top){
            this.leftSp =left;
            this.topSp =top;
        return  this;
    }

    GridFrame setGridSize(float width, float height){
        this.width =width;
        this.height =height;
        return this;
    }

    GridFrame setGridShape(int columns, int rows){
        return setGridShape(columns,rows,(columns*rows));
    }
    GridFrame setGridShape(int columns, int rows, int cells){
        this.rows=rows;
    ...
    /*
    private static float[] createUvCoordinates(GridFrame grid, int cell){
        int columns = grid.columns;
        int rows=grid.rows;

        float textureW = grid.getTexture().getWidth();
        float textureH = grid.getTexture().getHeight();

        float uGridW =  grid.width /textureW;
        float vGridH =  grid.height /textureH;
        float uWidthFrame=uGridW/columns;
        float vHeightFrame=vGridH/rows;

        float uOffsetW =  grid.leftSp /textureW;
        float vOffsetH =  grid.topSp /textureH;
        ...
        float bottom = top + vHeightFrame;
        float[] uvc;
        uvc = new float[FRAME_COORDINATES_SIZE];
        uvc[0]= left;
        uvc[1]=right;
        uvc[2]=top;
        uvc[3]=bottom;
